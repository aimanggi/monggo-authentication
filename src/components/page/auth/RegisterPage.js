...
class Popup extends React.Component {
  ...
  register = (e) => {
    ...
    .then((responseJson) => {
        ...
        console.log(this.state.emailSame)
...
render() {
    ...
    let emailSameWarn;
    if (emailSame) {
          emailSameWarn = <p>Maaf, email sudah terdaftar.</p>
        }
    const userCreated = this.state.userCreated;
    let userCreatedWarn;
    if (userCreated) {
            userCreatedWarn = <p>Registrasi berhasil! Silahkan Masuk untuk melanjutkan!</p>
        }
         ...
         <div className="modal-bg reg">
            <div className="signup">
                    ...
                    <div className="login-link">
                        <p className="text-center">Sudah Punya Akun ?<a className="log-link" href="#login" onClick={this.props.handleLogin}> Masuk Disini !</a></p>
                    </div>

                </div>
            </div>
            <Link className="redirect-home" exact to='/' style={{marginTop: '600px', color: 'white'}}>Beranda</Link>  
            </div>
        </div>
